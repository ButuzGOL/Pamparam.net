...
require_once( trailingslashit( dirname( __FILE__ ) ) . 'taxonomy-images-public-filters.php' );

function taxonomy_image_plugin_registr($taxonomies) {
    if (!is_array($taxonomies))
        $taxonomies = array($taxonomies);
    update_option('taxonomy_image_plugin_settings', array('taxonomies' => $taxonomies));
}
...
function taxonomy_image_plugin_modal_button( $fields, $post ) {
    if ( isset( $fields['image-size'] ) && isset( $post->ID ) && !(int)$_GET['post_id']  ) {
...
function taxonomy_image_plugin_get_image_src( $id ) {
	...
	if ( is_admin() ) {
		...
		return taxonomy_image_plugin_url( 'images/default.png' );
	}
	 ...
	 */
	return taxonomy_image_plugin_url( 'images/blank.png' );
}
...
function taxonomy_image_plugin_edit_tags_js() {
	...
	wp_localize_script( 'taxonomy-image-plugin-edit-tags', 'taxonomyImagesPlugin', array (
		...
		'img_src'  => taxonomy_image_plugin_url( 'images/default.png' ),
		'tt_id'    => 0,
		'image_id' => 0,
		) );
}
...
/**
 * Thickbox styles and scripts.
 *
 * @since     0.7
 * @access    private
 */
function taxonomy_image_plugin_css_and_javascript_thickbox() {
	if ( false == taxonomy_image_plugin_is_screen_active() ) {
		return;
	}
    ...
    wp_enqueue_script(
		'taxonomy-image-plugin-thickbox-resize',
		taxonomy_image_plugin_url( 'js/thickbox-resize.js' ),
		array( 'jquery', 'thickbox' ),
		taxonomy_image_plugin_version()
	);
}
add_action( 'admin_print_styles-edit-tags.php', 'taxonomy_image_plugin_css_and_javascript_thickbox' );
