...
class Wickett_Twitter_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
        ...
        $show_follow = (bool) $instance['showfollow'];
		...
		if ( 'error' != $tweets ) {
			...
			foreach ( (array) $tweets as $tweet ) {
				...
				echo "<li>{$before_tweet}{$text}{$before_timesince}<a href=\"" . esc_url( "http://twitter.com/{$account}/statuses/{$tweet_id}" ) . '" title="'.date("g:i A M jS", strtotime($tweet['created_at'])).'" class="timesince">' . str_replace(' ', '&nbsp;', wpcom_time_since(strtotime($tweet['created_at']))) . "&nbsp;ago</a></li>\n";
            ...
            if ($show_follow) {
                echo '<a href="https://twitter.com/'.$account.'" class="twitter-follow-button">Follow @'.$account.'</a>';
	...
	function update( $new_instance, $old_instance ) {
        ...
        $instance['showfollow'] = isset($new_instance['showfollow']);
	...
	function form( $instance ) {
        ...
        echo '<p><label for="' . $this->get_field_id('showfollow') . '"><input id="' . $this->get_field_id('showfollow') . '" class="checkbox" type="checkbox" name="' . $this->get_field_name('showfollow') . '"';
		if ( $show_follow )
			echo ' checked="checked"';
		echo ' /> ' . esc_html__('Show follow') . '</label></p>';
